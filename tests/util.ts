...
export async function init (page: Page) {
  ...
  await page.evaluate(() => {
    window.setTheme(2)
    window.cppCalls = []
    window._resize = (dx: number, dy: number, panelX: number, panelY: number, width: number, height: number, dragging: boolean) => {
      window.cppCalls.push({
        resize: [dx, dy, panelX, panelY, width, height, dragging]
      })
    }
