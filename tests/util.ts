...
export async function init (page: Page) {
  ...
  await page.evaluate(() => {
    ...
    window.cppCalls = []
    window.fcitx._resize = (dx: number, dy: number, anchorTop: number, anchorRight: number, anchorBottom: number, anchorLeft: number, fullWidth: number, fullHeight: number, dragging: boolean) => {
